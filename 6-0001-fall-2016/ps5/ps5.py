...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self,text):
        ...
        if self.phrase in new_text:
            n=new_text.find(self.phrase)
            if n+len(self.phrase)>=len(new_text):
                return True
            else:
                if new_text[n+len(self.phrase)]!=' ':
                    return False
                else:
                    return True
        else:
...
class BeforeTrigger(TimeTrigger):
    def evaluate(self,stoy):
        return self.pubtime>stoy.get_pubdate().replace(tzinfo=pytz.timezone("EST"))
